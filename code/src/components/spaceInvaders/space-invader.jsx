...
const SpaceInvaders = ({ screenChanger }) => {
  ...
  const moveBullets = () => {
    setBullets((prevBullets) =>
      prevBullets
        .map((bullet) => ({ ...bullet, y: bullet.y + BULLET_SPEED })) // Mueve los proyectiles hacia arriba (reduce y)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '20px' }}>
      ...
      <div style={{ width: CANVAS_WIDTH, height: CANVAS_HEIGHT, position: 'relative', overflow: 'hidden', backgroundImage: `url(${backgroundImg})`, backgroundSize: 'cover' }}>
        {/* Jugador */}
        <div
          style={{
            position: 'absolute',
